...
def get_chart(data, show_true_vals=True):
    ...
    hover = alt.selection_single(
        fields=["datetime"], nearest=True, on="mouseover", empty="none",
    )
    ...
    if show_true_vals:
        ...
        lines = (
            ...
            .mark_line()
            .encode(
                x=alt.X("datetime", title="Date"), y=alt.Y("load", title="Load in Kwh"),
            )
        )
...
def get_pred_data(n_days: int, model_name: str):
    ...
    plot_data = pd.DataFrame(
        {
            "datetime": pred_data.index.values,
...
def plot_gam_partial_dependence(feature_idx, feature_name):
    try:
        model: gam_model.LinGam = utils.load_model(
            root_dir.joinpath("models/linear-gam-model.pkl")
        )
    ...
    ax.plot(
        XX[:, model_term_idx],
        model.partial_dependence(term=model_term_idx, X=XX, width=0.95)[1],
        c="r",
        ls="--",
    )
