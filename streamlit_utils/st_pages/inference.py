...
MODEL_CHOICES = {
    "Gen. Additive Model (GAM)": "linear_gam",
...
def gam_partial_dependence_ui():
    ...
    st.write(
        "The plots show how each selected variable (feature) affects the model's prediction"
    )
    feature_idx_dict = {"Temperature": 6, "hour of day": 7, "month of the year": 8}
    ui_select_feature = st.selectbox(
        label="Select feature to view partial dependence plot:",
        options=list(feature_idx_dict.keys()),
    )
    feature_idx = feature_idx_dict[ui_select_feature]
    fig = st_utils.plot_gam_partial_dependence(
        feature_idx=feature_idx, feature_name=ui_select_feature
    )
...
GAM_FEATURE_COMMENTS = {
    "Temperature": f"The result show that the Temperature has a non-linear relationship with the model output. \
