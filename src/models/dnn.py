...
def adapt_scaler(X_train, y_train):
    ...
    utils.save_value(
        (X_scaler, y_scaler), fname=root_dir.joinpath("models/norm_Xy_scaler.pkl")
    )
...
def get_scaler():
    ...
    X_scaler, y_scaler = utils.load_value(
        fname=root_dir.joinpath("models/norm_Xy_scaler.pkl")
    )
...
def train_model(
    ...
    loss="huber",
    early_stop=False,
) -> tuple[DNN, tf.keras.callbacks.History]:
    ...
    train_data = window_dataset(
        X_train, y_train, window_size=window_size, batch_size=batch_size
    )
    ...
    if val_data is not None:
        ...
        val_data = window_dataset(
            X_test, y_test, window_size=window_size, batch_size=batch_size
        )
