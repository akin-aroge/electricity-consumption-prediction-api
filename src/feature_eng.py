...
def drop_collinear_cols(df, cols, thresh, training=True) -> pd.DataFrame:
    ...
    if training:
        utils.save_value(
            cols_to_drop, root_dir.joinpath("feature_store/temp_cols_to_drop.pkl")
        )
        utils.save_value(
            select_cols, root_dir.joinpath("feature_store/select_temp_cols.pkl")
        )
...
def make_train_features(df: pd.DataFrame, training=True, **kwargs) -> pd.DataFrame:
    ...
    raw_temp_cols = utils.load_value(
        root_dir.joinpath("feature_store/proc_temp_col_names.pkl")
    )
    # select temp cols
    df = df.pipe(drop_collinear_cols, cols=raw_temp_cols, thresh=temp_corr_thresh)
    ...
    if training:
        utils.save_value(
            col_names, root_dir.joinpath("feature_store/train_ft_col_names.pkl")
        )
...
def make_featured_data(
    df: pd.DataFrame, training: bool, drop_temp_cols: bool, **kwargs
):
    ...
    if (
        training
    ):  # additional columns available at training, but not before inference time
        df = df.pipe(feature_eng.feature_lag_nhr, feature="load", n=1).pipe(
            feature_eng.feature_lag_nhr, feature="load", n=2
        )
        train_col_names = list(df.columns.values).remove("load")
        utils.save_value(
            train_col_names, root_dir.joinpath("feature_store/train_ft_col_names.pkl")
        )
        df.to_csv(path_or_buf=root_dir.joinpath("feature_store/hist_feat_data.csv"))
