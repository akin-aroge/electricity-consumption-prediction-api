...
def get_raw_load_data(dir_path: str, max_year: int, training=True) -> pd.DataFrame:
    ...
    if training:
        utils.save_value(
            col_names, root_dir.joinpath("feature_store/raw_load_data_col.pkl")
        )
...
def get_raw_temp_data(dir_path: str, max_year=None, training=True) -> pd.DataFrame:
    ...
    if training:
        # store col names
        utils.save_value(
            temp_cols, root_dir.joinpath("feature_store/proc_temp_col_names.pkl")
        )
...
def _reshape_temp_df(temp_data_raw: pd.DataFrame):
    temp_data_raw["datetime"] = temp_data_raw.date + pd.to_timedelta(
        temp_data_raw.hr, unit="h"
    )
    temp_data_raw["dummy_col_01"] = np.arange(len(temp_data_raw)) % 2

    temp_data_pivot = pd.pivot_table(
        temp_data_raw,
        values="temperature",
        index=["date", "hr", "datetime", "dummy_col_01"],
        columns="station_id",
    )
    ...
    # rename columns
    str_temp_cols_dict = dict(
        [
            (col, "t" + str(col))
            for col in temp_data_pivot.columns
...
def init_dataset(load_dir, temp_dir, max_year: int, training=True) -> pd.DataFrame:
    ...
    raw_temp_data = raw_temp_data.pipe(_add_datatime_col).drop(["date", "hr"], axis=1)
