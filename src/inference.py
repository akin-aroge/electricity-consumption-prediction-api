...
def return_old_temps(dates, return_mean_temp=False) -> pd.DataFrame:
    ...
    temp_ft_names = utils.load_value(
        root_dir.joinpath("feature_store/select_temp_cols.pkl")
    )
    ...
    for name in temp_ft_names:
        df[name] = df.datetime.apply(
            lambda date: get_past_hr_ft(
                lookup_dt=feature_eng.back_n_yr(date, n=1),
                feature_name=name,
                past_data=past_data,
            )
        )
...
def select_temp_cols(temp_input):
    select_temp_ft_names = utils.load_value(
        root_dir.joinpath("feature_store/select_temp_cols.pkl")
    )
    ...
    temp_cols = np.concatenate((select_temp, mean_temp.reshape(-1, 1)), axis=1)

    temp_cols = pd.DataFrame(
        data=temp_cols, columns=select_temp_ft_names + ["mean_temp"]
    )
...
def order_inf_data_cols(data: pd.DataFrame):
    pre_inf_cols = utils.load_value(
        root_dir.joinpath("feature_store/pre_inf_train_ft_col_names.pkl")
    )
...
def get_input():
    ...
    future_dates = future_temp_data.date + pd.to_timedelta(
        future_temp_data.hr, unit="h"
    )
    future_dates = future_dates.values
    future_temps = future_temp_data.iloc[:, 2:].values

    inf_df = init_df(future_dates, future_temps)
    inf_df = feature_eng.make_featured_data(
        inf_df, training=False, drop_temp_cols=False
    )
...
def get_dnn_predictions(inf_df: pd.DataFrame):
    ...
    dnn_train_params = utils.load_value(
        fname=root_dir.joinpath("models/train_params_dnn.pkl")
    )
    ...
    pred_data = pd.DataFrame({"datetime": inf_df.index.values, "load": pd.NA})
    ...
    for i in range(len(pred_data)):
        ...
        load_prev_1h = get_hr_ft(
            prev_1hr, "load", pred_data=pred_data, past_data=past_data
        )
        load_prev_2h = get_hr_ft(
            prev_2hr, "load", pred_data=pred_data, past_data=past_data
        )
...
def get_gam_predictions(df: pd.DataFrame):
    ...
    pred_data = pd.DataFrame({"datetime": df.index.values, "load": pd.NA})
    pred_data.set_index("datetime", inplace=True)
...
def get_pred_error(pred_load):
    actual_vals = pd.read_csv(
        root_dir.joinpath("data/eval_data/Solution.csv"), parse_dates=[0]
    )
    actual_vals["datetime"] = actual_vals.Date + pd.to_timedelta(
        actual_vals.Hour, unit="h"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_name", "-m", type=str, required=True, help="specify model type"
    )
